...
var matchScore,matchAdress,fLayerObjId, lati, longi;
  ...
  return declare([], {
    ...
    _validateField: function (currentNode, iskeyPress) {
      var inputType, inputValue, displayType = null,
        ...
        decimalAndBlank = /^(\d*|\d+\.\d{0,3}|\d*\.\d{1,3})$/,
      ...
      //Change in logic made for North Carolina broadband application
      //using regex to allow only decimal number or blank in text input field
    if (inputValue.length !== 0 && ((displayType === "email" && inputValue.match(email)) || (displayType === "url" && inputValue.match(url)) ||displayType === null) || (displayType === "text" && inputValue.match(decimalAndBlank)) || displayType === "textarea") {
      ...
      arcgisUtils.createMap(itemInfo, mapDiv, {
        ...
        if (submitButtonNode) {
          on(submitButtonNode, a11yclick, lang.hitch(this, function () {
            this._submitForm();
            //window.alert("line 1777");
              //window.alert(dojo.byId("search-widget_input").innerHTML);
               //alert(document.getElementById("search-widget_input").value);
    ...
    _createGeocoderButton: function () {
      ...
      on(this.geocodeAddress, "select-result", lang.hitch(this, function (evt) {
        ...
        var coords = this._calculateLatLong(evt.result.feature.geometry);
         lati = coords.substr(10, 8);
         longi = coords.substr(37, 9);
        //window.alert(evt.result.feature.attributes.score);
        matchScore = (evt.result.feature.attributes.score);
        //window.alert(evt.result.feature.attributes.Match_addr);
        matchAdress= (evt.result.feature.attributes.Match_addr);
    ...
    _addFeatureToLayer: function () {
      ...
      //Form fields from the user inputs loops are looped through and added to featureData
      ...
      fLayerObjId = (addResults[0].objectId);

      //Pass to bb-custom.js
      captureGeoCodeOutputResults();
        //alertOne();
